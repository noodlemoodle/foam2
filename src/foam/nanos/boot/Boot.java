...
public class Boot {
  ...
  public Boot(String datadir) {
    ...
    ((ProxyDAO) root_.get("nSpecDAO")).setDelegate(
        new foam.nanos.auth.AuthorizationDAO(root_, serviceDAO_, new foam.nanos.auth.StandardAuthorizer("service")));
    // 'read' authenticated version - for dig and docs
    ((ProxyDAO) root_.get("AuthenticatedNSpecDAO")).setDelegate(
        new foam.dao.PMDAO(root_, new foam.nanos.auth.AuthorizationDAO(root_, (DAO) root_.get("nSpecDAO"), new foam.nanos.auth.StandardAuthorizer("service"))));
